...
<head>
  <meta charset="utf-8" />
  <title>{{ company }} × {{ country }} 전략 카드</title>
  <style>
...
<body>
  <div class="wrap">
    <div class="header">
      <h1>{{ company }} × {{ country }} 전략 카드</h1>
      <div class="meta">결정: <strong>{{ decision.status }}</strong> ({{ decision.reason }})
        <span class="badge {% if risk_badge=='높음' %}high{% elif risk_badge=='보통' %}mid{% else %}low{% endif %}">Risk {{ risk_badge or 'N/A' }}</span>
    ...
    <div class="card" style="margin-top:16px">
      <h2>GTM</h2>
      <div>선택: <strong>{{ gtm_selected }}</strong></div>
